...
export function resolveEdge(ctx) {
  const sys = 'You are a fact deduplication assistant. NEVER mark facts with key differences as duplicates.';
  const user = `
...
<EXISTING FACTS>
${toPromptJson(ctx.existing_edges)}
</EXISTING FACTS>

<FACT INVALIDATION CANDIDATES>
${toPromptJson(ctx.edge_invalidation_candidates)}
</FACT INVALIDATION CANDIDATES>

<NEW FACT>
${toPromptJson(ctx.new_edge)}
</NEW FACT>
...
`;
  return { system: sys, user, schema: 'resolve_edge' };
}
